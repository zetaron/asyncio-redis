...
from asyncio_redis.exceptions import ConnectionLostError, TimeoutError
...
class RedisProtocolTest(TestCase):
    ...
    def test_extended_set(self, transport, protocol):
        ...
        value = yield from protocol.set(
            u"my_key", u"my_value", expire=10, only_if_not_exists=True
        )
        self.assertEqual(value, StatusReply("OK"))
        ...
        value = yield from protocol.set(
            u"my_key", u"other_value", pexpire=20000, only_if_exists=True
        )
        self.assertEqual(value, StatusReply("OK"))
    ...
    def test_keys(self, transport, protocol):
        ...
        self.assertEqual(
            set(all_keys), {"our-keytest-key1", "our-keytest-key2", "our-keytest-key3"}
        )
    ...
    def test_bitops(self, transport, protocol):
        ...
        bytes_transport, bytes_protocol = yield from connect(
            self.loop, lambda **kw: RedisProtocol(encoder=BytesEncoder(), **kw)
        )
    ...
    def test_zset(self, transport, protocol):
        ...
        self.assertEqual(
            (yield from result.asdict()), {"key": 4.0, "key2": 5.0, "key3": 5.5}
        )
        ...
        self.assertEqual((yield from result.asdict()), {"key2": 5.0, "key3": 5.5})
        result = yield from protocol.zrange("myzset", -2, -1)
        self.assertIsInstance(result, ZRangeReply)
        ...
        result = yield from protocol.zrevrangebyscore(
            "myzset", max=ZScoreBoundary(5.5, exclude_boundary=True)
        )
        self.assertEqual((yield from result.asdict()), {"key": 4.0, "key2": 5.0})
    ...
    def test_zset_zincrby(self, transport, protocol):
        ...
        result = yield from protocol.zrange("myzset")
        self.assertEqual(
            (yield from result.asdict()), {"key": 5.1, "key2": 5.0, "key3": 5.5}
        )
    ...
    def test_password(self, transport, protocol):
        ...
        transport2, protocol2 = yield from connect(
            self.loop, lambda **kw: RedisProtocol(password="newpassword", **kw)
        )
...
class RedisConnectionTest(TestCase):
    ...
    def test_connection(self):
        ...
        def test():
            ...
            self.assertEqual(
                repr(connection), "Connection(host=%r, port=%r)" % (HOST, PORT)
            )
...
class RedisPoolTest(TestCase):
    ...
    def test_pool(self):
        ...
        def test():
            ...
            self.assertEqual(
                repr(connection), "Pool(host=%r, port=%r, poolsize=1)" % (HOST, PORT)
            )
    ...
    def test_connection_lost(self):
        ...
        def test():
            ...
            with self.assertRaises(NoAvailableConnectionsInPoolError) as e:
                yield from connection.set("key", "value")
            self.assertIn(
                "No available connections in the pool: size=1, in_use=0, connected=0",
                e.exception.args[0],
            )
